                    ...
                    <div class="block{if $zbp->Config('tpure')->PostARCHIVEFOLDON == '1'} fold{/if}">
                        <div class="post">
                            ...
                            <div class="single{if (($type=='article' && $article.Metas.viewall != '1' && $zbp->Config('tpure')->PostVIEWALLSINGLEON)||($type=='page' && $article.Metas.viewall != '1' && $zbp->Config('tpure')->PostVIEWALLPAGEON))} viewall{/if}">
                                {if $zbp->Config('tpure')->PostARCHIVEINFOON == '1'}<div class="listintro">{$lang['tpure']['bynow']}{date("Y年m月d日")}，{$lang['tpure']['postall']}{$zbp->cache->all_article_nums}{$lang['tpure']['articles']}。<a href="javascript:;" data-foldtext="{$lang['tpure']['foldoff']}" class="archivefold{if $zbp->Config('tpure')->PostARCHIVEFOLDON == '1'} on{/if}">{$lang['tpure']['foldon']}</a></div>{/if}
                                {tpure_GetArchives()}
                            </div>
                        </div>
                    </div>
