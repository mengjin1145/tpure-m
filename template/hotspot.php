{* Template Name:列表页热点样式模板 * Template Type:list,author *}
<!DOCTYPE html>
                    ...
                    {if $zbp->Config('tpure')->PostBANNERSEARCHON}
                        <div class="hellosch{if !$zbp->Config('tpure')->PostBANNERFONT} alone{/if}">
                            <form name="search" method="post" action="{$host}zb_system/cmd.php?act=search">
                                <input type="text" name="q" placeholder="{$zbp->Config('tpure')->PostSCHTXT}" autocomplete="off" class="helloschinput">
...
{/if}
            ...
            <div{if $zbp->Config('tpure')->PostFIXSIDEBARSTYLE == '0'} id="sticky"{/if}>
                <div class="content listcon">
                    ...
                    {/if}
                    <div class="block custom{if $zbp->Config('tpure')->PostBIGPOSTIMGON=='1'} large{/if} hotspot">
                        ...
                        <div class="filternav">
                            <form id="filternav">
                                ...
                                <input type="hidden" name="order" value="{htmlspecialchars(GetVars('order','GET'), ENT_QUOTES, 'UTF-8')}">
                                <input type="hidden" name="sort" value="{htmlspecialchars((int)GetVars('sort','GET'), ENT_QUOTES, 'UTF-8')}">
                            </form>
                        </div>
    <script>
        !function(f){
