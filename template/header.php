...
{if $zbp->Config('tpure')->PostSHAREARTICLEON == '1' || $zbp->Config('tpure')->PostSHAREPAGEON == '1'}
    ...
    <script defer src="{$host}zb_users/theme/{$theme}/plugin/share/share.js"></script>
{/if}
{if $zbp->Config('tpure')->PostSLIDEON == '1'}
    ...
    <script defer src="{$host}zb_users/theme/{$theme}/plugin/swiper/swiper.min.js"></script>
{/if}
    <!-- 🚀 优化：主样式表异步加载（避免渲染阻塞） -->
    ...
    <noscript><link rel="stylesheet" href="{$host}zb_users/theme/{$theme}/style/{$style}.css?v={$zbp->themeapp->version}"></noscript>
{if $zbp->Config('tpure')->PostCOLORON == '1'}
    <link rel="preload" href="{$host}zb_users/theme/{$theme}/include/skin.css" as="style" onload="this.onload=null;this.rel='stylesheet'">
    <noscript><link rel="stylesheet" href="{$host}zb_users/theme/{$theme}/include/skin.css"></noscript>
{/if}
    <!-- jQuery 核心库（必须同步加载） -->
    <script src="{$host}zb_system/script/jquery-latest.min.js"></script>
    <!-- 🚀 优化：非关键 JS 延迟加载 -->
    <script defer src="{$host}zb_system/script/zblogphp.js"></script>
    <script defer src="{$host}zb_system/script/c_html_js_add.php"></script>
{if $type == 'article'}
...
{/if}
    <!-- 🚀 优化：主题脚本延迟加载 -->
    <script defer src="{$host}zb_users/theme/{$theme}/script/common.js?v={$zbp->themeapp->version}"></script>
