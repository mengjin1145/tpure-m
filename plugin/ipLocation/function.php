<?php
function tpure_getIP($ip) {
    global $zbp;
    $location = array();
    $filePath = $zbp->path.'zb_users/theme/tpure/plugin/ipLocation/qqwry.dat';
    if(!file_exists($filePath)){
        $zbp->Config('tpure')->PostCMTIPON = '0';
        $zbp->SaveConfig('tpure');
    }else{
        $iplocation = new tpure_qqwry($filePath);
        $location = $iplocation->getlocation($ip);
    }
    return $location;
}

function tpure_ipLocation($ip)
{
    $ipdetail = tpure_getIP($ip);
    if (!is_array($ipdetail) || !isset($ipdetail['country'])) {
        return "未知";
    }
    $country = $ipdetail['country'];
    if (strpos($country, '中国') === 0) {
        $region = explode('–', $country);
        if (in_array($region[1], ['北京', '上海', '天津', '重庆'])) {
            return $region[1];
        } else {
            return str_replace('省', '', $region[1]);
        }
    } else {
