...
function tpure_CmdAjax($src) {
    ...
    // 在线人数统计
    if ($src === 'tpure_stats_online') {
        tpure_ajax_stats_online();
    }
}
 ...
 * @return void
 */
function tpure_ajax_stats_online() {
    // 清除之前可能的输出
    ob_clean();
    ...
    if (!class_exists('TpureStatistics')) {
        echo json_encode(array(
            'success' => false,
            'count' => 0,
            'error' => 'TpureStatistics class not found'
        ));
        exit;
    }
    
    try {
        $count = TpureStatistics::getOnlineCount();
