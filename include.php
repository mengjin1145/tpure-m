...
if (!defined('ZBP_IN_ADMIN') && !isset($_COOKIE['username']) && $_SERVER['REQUEST_METHOD'] === 'GET') {
    // 🔧 读取缓存配置（需要提前加载 Z-BlogPHP 配置）
    $fullPageCacheEnabled = false;
    $configFile = dirname(__FILE__) . '/../../cache/config_tpure.php';
    if (file_exists($configFile)) {
        $tpureConfig = @include $configFile;
        if (is_array($tpureConfig) && isset($tpureConfig['CacheFullPageOn'])) {
            $fullPageCacheEnabled = ($tpureConfig['CacheFullPageOn'] === 'ON');
        }
    }
    ...
    if ($fullPageCacheEnabled && $isCacheable && extension_loaded('redis')) {
        try {
...
if (!function_exists('tpure_esc_attr')) {
    /**
     * HTML 属性值安全转义
     * @param string $text 属性值
     * @return string 转义后的属性值
     */
    function tpure_esc_attr($text) {
...
function ActivePlugin_tpure() {
    ...
    if (!defined('ZBP_IN_ADMIN') || !ZBP_IN_ADMIN) {
        ...
        if (function_exists('tpure_register_cache_hooks')) {
            // 检查热门内容缓存是否启用
            $hotCacheEnabled = isset($zbp->Config('tpure')->CacheHotContentOn) && $zbp->Config('tpure')->CacheHotContentOn === 'ON';
            if ($hotCacheEnabled) {
    ...
    // 🔧 修复：后台顶部菜单钩子必须无条件注册（Z-BlogPHP 内部会判断是否为后台）
