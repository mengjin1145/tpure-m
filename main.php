        ...
        <dd class="half">
            ...
            <?php
                ...
                if(!file_exists($filePath)){
                    echo '<a href="https://www.toyean.com/ipdb.html" target="_blank" class="downbtn">点此下载IP库</a><input type="hidden" name="PostCMTIPON" value="0">';
                }else{
                    echo '<input type="text" id="PostCMTIPON" name="PostCMTIPON" class="checkbox" value="'.$zbp->Config('tpure')->PostCMTIPON.'">';
                }
            ?>
            <i class="help"></i><span class="helpcon">“ON”为文章页面评论模块显示评论者IP归属；<br>“OFF”为文章页面评论模块不显示评论者IP归属。<br>请将下载的qqwry.dat文件通过FTP或文件管理上传至：<em>zb_users/theme/tpure/plugin/ipLocation/<b>qqwry.dat</b></em></span>
        </dd>
        <dd>
...
<dl>
    ...
    <form method="post" class="setting">
        ...
        <dt>缓存性能优化 <span>(缓存可显著提升网站性能，建议按需开启)</span></dt>
        <dd class="half">
            <label>Redis 全页面缓存</label>
            <input type="text" id="CacheFullPageOn" name="CacheFullPageOn" class="checkbox" value="<?php echo $zbp->Config('tpure')->CacheFullPageOn; ?>">
            <i class="help"></i><span class="helpcon">"ON"为开启 Redis 全页面缓存（需安装 zbpcache 插件并配置 Redis）；<br>"OFF"为关闭全页面缓存；<br><em>开启后可极大提升页面加载速度（1000ms+ → 50ms），适合高流量网站。</em></span>
        </dd>
        <dd class="half">
            <label>热门内容 HTML 缓存</label>
            <input type="text" id="CacheHotContentOn" name="CacheHotContentOn" class="checkbox" value="<?php echo $zbp->Config('tpure')->CacheHotContentOn; ?>">
            <i class="help"></i><span class="helpcon">"ON"为开启热门文章、评论等模块的 HTML 缓存（需 Redis）；<br>"OFF"为每次动态查询数据库；<br><em>开启后可减少数据库查询，提升侧边栏加载速度。</em></span>
        </dd>
        <dd class="half">
            <label>浏览器缓存（HTTP）</label>
            <input type="text" id="CacheBrowserOn" name="CacheBrowserOn" class="checkbox" value="<?php echo $zbp->Config('tpure')->CacheBrowserOn; ?>">
            <i class="help"></i><span class="helpcon">"ON"为启用浏览器缓存（Cache-Control、ETag）；<br>"OFF"为禁用浏览器缓存；<br><em>开启后静态资源可缓存 30 天，减少重复下载。</em></span>
        </dd>
        <dd class="half">
            <label>模板缓存</label>
            <input type="text" id="CacheTemplateOn" name="CacheTemplateOn" class="checkbox" value="<?php echo $zbp->Config('tpure')->CacheTemplateOn; ?>">
            <i class="help"></i><span class="helpcon">"ON"为启用 Z-BlogPHP 原生模板缓存（推荐）；<br>"OFF"为关闭模板缓存；<br><em>模板缓存可避免重复编译模板文件，建议保持开启。</em></span>
        </dd>
